...
class SelectionModal(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title: str, options: list, on_apply: callable):
        ...
        ctk.CTkLabel(self.container, text=title, font=(FONT_FAMILY, 18, "bold"), text_color=COLOR_TEXT).pack(pady=(0, 20))

        # Scrollable Options
        self.scroll = ctk.CTkScrollableFrame(self.container, fg_color=COLOR_SURFACE)
        ...
        ctk.CTkButton(btn_frame, text="Apply", command=self.apply, **get_button_style("filled-primary")).pack(side="right", padx=5)
...
class RTXSettingsModal(SelectionModal):
    ...
    def __init__(self, parent, on_apply_config: callable):
        ...
        options = [
            {"label": "Disable Dithering (Blocks)", "value": "disable_dithering_blocks", "checked": True},
            {"label": "Disable Dithering (Mobs)", "value": "disable_dithering_mobs", "checked": True},
            {"label": "Force Graphics Mode 3 (Deferred)", "value": "force_graphics_mode_3", "checked": True},
            {"label": "Enable Graphics Switch", "value": "enable_graphics_switch", "checked": True},
        ]
...
class OptionsFilePickerModal(ctk.CTkToplevel):
    ...
    def _browse_file(self):
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[("Options File", "options.txt"), ("Text Files", "*.txt"), ("All Files", "*.*")]
        )
...
class AllSettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, config_data: dict, on_save: callable):
        ...
        ctk.CTkLabel(self.container, text="Edit Configuration", font=(FONT_FAMILY, 20, "bold")).pack(pady=(0, 20))
        ...
        ctk.CTkButton(btn_frame, text="Save Changes", command=self.save, **get_button_style("filled-primary")).pack(side="right", padx=5)
        ctk.CTkButton(btn_frame, text="Close", command=self.destroy, fg_color="transparent", border_width=1, border_color="gray").pack(side="right", padx=5)
    ...
    def _build_form(self, data, parent_frame, prefix=""):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, bool):
                ...
                self.widgets[full_key] = var
                ctk.CTkEntry(row, textvariable=var, width=200).pack(side="right")
                ...
                ctk.CTkEntry(row, textvariable=var, width=200).pack(side="right")
                 ...
                 self.widgets[full_key] = var
                 ctk.CTkEntry(row, textvariable=var, width=200).pack(side="right")
                 ctk.CTkLabel(row, text="(List)", font=("Unk", 10)).pack(side="right", padx=5)

            elif isinstance(value, dict):
                 # Recurse? Nested dicts in flat list might be messy.
                 # Let's add a Label and indent
                 ctk.CTkLabel(row, text="(Group)", text_color="gray").pack(side="right")
                 
                 sub_frame = ctk.CTkFrame(parent_frame, fg_color="#222222") # Slightly darker
             ...
             for key, val in target_dict.items():
                 ...
                 if full_key in self.widgets:
                     ...
                     if isinstance(val, bool):
                         ...
                         target_dict[key] = [x.strip() for x in str(raw_val).split(",") if x.strip()]
