...
def get_pack_files(pack_path, num_threads=None, set_total_callback=None, progress_callback=None, status_callback=None, keep_temp=False):
    files_data = {}
    processed = 0
    ...
    try:
        ...
        if status_callback: status_callback("Copying/Extracting pack to temporary folder...")
        ...
        if status_callback: status_callback("Extracting .brarchive files...")
        brarchives = []
        ...
        def extract_brarchive(br_path):
            ...
            extract_dir = os.path.join(os.path.dirname(br_path), f"{base_name}_extracted")
            try:
        ...
        if brarchives:
            success_count = 0
            fail_count = 0
            if status_callback: status_callback(f"Extracting {len(brarchives)} .brarchive files...")
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    if future.result():
                        ...
                        fail_count += 1
            if status_callback: status_callback(f"Extracted {success_count} brarchives ({fail_count} failed)")
                
        # STEP 3: Hash all the files
        if status_callback: status_callback("Calculating file hashes...")
        tasks = []
        for root, dirs, files in os.walk(temp_root):
            for file in files:
        ...
        if set_total_callback: set_total_callback(len(tasks))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                processed += 1
                if progress_callback: progress_callback(processed)
...
class DiffCheckerApp(ttk.Window):
    ...
    def create_widgets(self):
        ...
        thread_opts = ["Auto"] + [str(i) for i in range(1, (os.cpu_count() or 4) * 2 + 1)]
        self.thread_combo = ttk.Combobox(ctrl_frame, textvariable=self.thread_var, values=thread_opts, width=5, state="readonly")
        self.thread_combo.pack(side=LEFT, padx=(0, 15))
        
        self.keep_temp_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(ctrl_frame, text="Keep Temp Folders", variable=self.keep_temp_var, bootstyle="round-toggle").pack(side=LEFT, padx=(0, 15))
        
        ttk.Button(ctrl_frame, text="Compare Packs", bootstyle=SUCCESS, command=self.compare_packs).pack(side=LEFT)
        ttk.Button(ctrl_frame, text="Copy Report", bootstyle=INFO, command=self.copy_logs).pack(side=LEFT, padx=5)
        ...
        ttk.Label(debug_frame, textvariable=self.debug_threads_var, font=('Consolas', 9)).pack(side=LEFT, padx=(0, 15))
        ttk.Label(debug_frame, textvariable=self.debug_elapsed_var, font=('Consolas', 9)).pack(side=LEFT, padx=(0, 15))
        ttk.Label(debug_frame, textvariable=self.debug_avg_var, font=('Consolas', 9)).pack(side=LEFT, padx=(0, 15))
        ttk.Label(debug_frame, textvariable=self.debug_eta_var, font=('Consolas', 9)).pack(side=LEFT)
        
        # Output
        self.output_text = ScrolledText(self, width=100, height=22, bg='#1e1e1e', fg='#d4d4d4', font=('Consolas', 10))
    ...
    def compare_packs(self):
        ...
        threading.Thread(target=self._run_comparison, args=(p1, p2, num_threads, keep_temp), daemon=True).start()

    def _run_comparison(self, p1, p2, num_threads, keep_temp):
        import time
        
        actual_threads = num_threads if num_threads is not None else (os.cpu_count() or 4) + 4
        ...
        def process_pack(p_num, path):
            ...
            def progress_cb(processed_val):
                ...
                if curr_time - last_update_time[0] > 0.06:
                    ...
                    self.after(0, lambda e=elapsed: self.debug_elapsed_var.set(f"Elapsed: {e:.1f}s"))
                    self.after(0, lambda a=avg_time: self.debug_avg_var.set(f"Avg/file: {a*1000:.1f}ms"))
                    self.after(0, lambda e_t=eta: self.debug_eta_var.set(f"ETA: {e_t:.1f}s"))
            ...
            res, temp_path = get_pack_files(path, num_threads, 
            ...
            self.after(0, lambda e=total_elapsed: self.debug_elapsed_var.set(f"Elapsed: {e:.1f}s"))
            self.after(0, lambda a=avg_final: self.debug_avg_var.set(f"Avg/file: {a*1000:.1f}ms"))
        ...
        if removed:
            lines.append("=== REMOVED FILES (Present in Pack 1, Missing in Pack 2) ===\n")
            for f in removed:
                lines.append(f" - {f}\n")
            lines.append("\n")
