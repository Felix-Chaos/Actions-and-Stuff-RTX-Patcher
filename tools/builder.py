...
def check_dependencies():
    ...
    for req in required:
        try:
            if req == "PyInstaller":
                subprocess.check_call([sys.executable, "-c", "import PyInstaller"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            elif req == "PIL":
                subprocess.check_call([sys.executable, "-c", "import PIL"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            else:
                subprocess.check_call([sys.executable, "-c", f"import {req}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def install_dependencies(missing_list):
    ...
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "pypiwin32"] + missing_list)
